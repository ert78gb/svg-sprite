...
SVGSpriter.prototype._transformShape = function(shape, cb) {
    ...
    const createTransformTask = transform => {
        ...
        if (Object.prototype.hasOwnProperty.call(this._shapeTransformers, transform[0]) && _.isObject(transform[1])) {
            return () => {
...
SVGSpriter.prototype.compile = function(config, cb) {
    const _config = _.isPlainObject(config) ?
        this.config.filter(config) : // eslint-disable-line unicorn/no-array-callback-reference
